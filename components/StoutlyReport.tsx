...
import { BarChart3, Map, Calendar, Beer, TrendingUp, TrendingDown, PoundSterling, Euro, Trophy, ArrowDownToLine, ArrowUpFromLine, CreditCard, Activity, Camera, X, ChevronLeft, ChevronRight, Info, Tag, Shield, Instagram } from 'lucide-react';
...
const worldCupPriceChanges: Record<string, number> = {
  "United Kingdom (All)": 11.24,
  "London": 3.48,
  "Northern Ireland": 3.47,
  "Belfast": 10.84,
  "Derry/Londonderry": 3,
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 flex flex-col justify-center gap-6">
                   <div className="border-b border-gray-700 pb-6">
                      ...
                      <div className="text-sm font-semibold mt-3 bg-gray-900/50 p-2 rounded-lg border border-gray-700 inline-block">
                        <span className="text-gray-400">During World Cup: </span>
                        <span className={worldCupPriceChanges[currentData.region] > 0 ? 'text-rose-400' : 'text-emerald-400'}>
                           {worldCupPriceChanges[currentData.region] > 0 ? '+' : ''}{worldCupPriceChanges[currentData.region]}%
                        </span>
                      </div>
                   </div>
                   <div>
        ...
        <div className="fixed inset-0 z-[100] bg-gray-950/95 backdrop-blur-md overflow-hidden flex flex-col items-center justify-center">
            ...
            <div className="absolute top-4 right-4 flex gap-4 z-[110]">
               <button 
                 ...
                 className={`p-3 rounded-full transition-colors shadow-lg ${isInstagram ? 'bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-500 text-white' : 'bg-gray-800 text-white hover:bg-gray-700'}`}
            ...
            <div className={`bg-gray-900 border border-gray-700 ${isInstagram ? 'p-3 pb-2 sm:p-4 sm:pb-3' : 'p-6 sm:p-8'} shadow-2xl shrink-0 flex flex-col relative overflow-hidden ring-1 ring-white/10 ${isInstagram ? 'w-[480px] aspect-[4/5] rounded-[2rem]' : 'w-[480px] min-h-[680px] rounded-[2rem]'}`}>
                {/* background accents */}
                ...
                {infographicPage === 1 ? (
                  ...
                  <div className={`grid flex-1 relative z-10 ${isInstagram ? 'grid-cols-3 gap-1.5' : 'grid-cols-4 gap-3'}`}>
                     ...
                     <div className={`bg-gray-800/80 rounded-3xl border border-gray-700 flex flex-col justify-center items-center text-center shadow-lg relative ${isInstagram ? 'col-span-1 p-2' : 'col-span-2 p-4'}`}>
                       ...
                       <div className={`text-gray-500 mt-1 max-w-[160px] leading-tight text-center px-1 ${isInstagram ? 'text-[7px]' : 'text-[8px] px-2'}`}>
                         (Quality + Price) × Confidence Rating
                       </div>
                       
                       {/* Worldwide Comparison */}
                       <div className={`mt-2.5 font-bold px-2 py-0.5 rounded-full flex items-center gap-1 border ${currentData.stats.avgScore >= 39.7 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'} ${isInstagram ? 'text-[7px] flex-col py-1 text-center leading-tight' : 'text-[9px] flex-row'}`}>
                         {currentData.stats.avgScore >= 39.7 ? <TrendingUp size={10} className="shrink-0" /> : <TrendingDown size={10} className="shrink-0" />}
                         <span>{Math.abs(currentData.stats.avgScore - 40).toFixed(1)} {currentData.stats.avgScore >= 40 ? 'above' : 'below'} global avg</span>
                       </div>
                     </div>
                       ...
                       {'top3RatedPubs' in currentData.stats && Array.isArray(currentData.stats.top3RatedPubs) ? (
                         <>
                           ...
                           <div className="w-full flex flex-col gap-1.5 z-10">
                             {currentData.stats.top3RatedPubs.slice(0,3).map((pub: typeof belfastData.stats.topRatedPub, i: number) => (
                               <div key={pub.id} className={`flex justify-between items-center bg-gray-900/40 rounded ${isInstagram ? 'px-2 py-0.5' : 'px-2 py-1'}`}>
                     ...
                     <div className={`bg-emerald-500/10 rounded-xl border border-emerald-500/30 flex flex-col justify-center items-center text-center col-span-1 ${isInstagram ? 'p-1.5' : 'p-3'}`}>
                       ...
                       <div className="text-emerald-500 font-bold text-[10px] leading-[1.1] mb-1 break-words line-clamp-3 w-full px-1">{currentData.stats.cheapestPub.name}</div>
                     ...
                     <div className={`bg-rose-500/10 rounded-xl border border-rose-500/30 flex flex-col justify-center items-center text-center col-span-1 ${isInstagram ? 'p-1.5' : 'p-3'}`}>
                       ...
                       <div className="text-rose-500 font-bold text-[10px] leading-[1.1] mb-1 break-words line-clamp-3 w-full px-1">{currentData.stats.expensivePub.name}</div>
                     ...
                     <div className={`bg-gray-800/80 rounded-xl border border-gray-700 flex flex-col justify-center items-center text-center col-span-1 ${isInstagram ? 'p-1.5' : 'p-3'}`}>
                       ...
                       <div className={`mt-1.5 text-[8px] font-bold ${worldCupPriceChanges[currentData.region] > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                         {worldCupPriceChanges[currentData.region] > 0 ? '+' : ''}{worldCupPriceChanges[currentData.region]}% World Cup
                       </div>
                     </div>
                  ...
                  <div className="flex-1 flex flex-col relative z-10">
                     <div className="bg-gray-800/80 rounded-2xl p-6 border border-gray-700 shadow-lg mb-4 flex-1 flex flex-col">
                       ...
                       <div className="w-full flex-1 min-h-[200px] -ml-4 relative mt-auto mb-4">
                         <ResponsiveContainer width="100%" height="100%">
                     ...
                     <div className={`grid mt-auto ${isInstagram ? 'grid-cols-1 gap-2' : 'grid-cols-2 gap-3'}`}>
                       ...
                       <div className="bg-gray-800/80 rounded-xl p-4 border border-gray-700 text-center flex flex-col justify-center">
                         ...
                         <div className={`mt-2 text-[10px] font-bold ${worldCupPriceChanges[currentData.region] > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                           {worldCupPriceChanges[currentData.region] > 0 ? '+' : ''}{worldCupPriceChanges[currentData.region]}% during World Cup
                         </div>
                       </div>
                     </div>
                  </div>
                ...
                {/* Footer */}
                <div className={`text-center relative z-10 mt-auto pt-2 ${isInstagram ? 'mb-1' : 'mb-0'}`}>
                  <div className={`flex items-center justify-center gap-2 font-bold tracking-wider text-amber-500/80 ${isInstagram ? 'mb-0 text-[10px]' : 'mb-1 text-sm'}`}>
